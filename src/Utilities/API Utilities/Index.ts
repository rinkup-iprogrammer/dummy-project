...
export const getCellInfo = async () => {
    ...
    if (permission) {
        NativeModules.SimInfo.getNetworkTypeCheck((response: any) => {
            isNetworkType = response;
        })
    }
};

export const serverCall = async (url: string, method: string, data: object, additionalHeader?: any, selectedUser?: any, isFromPromo = false) => new Promise(async (resolve, reject) => {
